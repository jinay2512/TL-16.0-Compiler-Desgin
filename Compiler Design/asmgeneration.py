...
class AssemblyGenerator(NodeVisitor):
    ...
    def visit_BinOp(self, node):
        ...
        self.fp_Stack.append(self.fp)  # pushed into stack, Popped at receiving end
        ...
        if type(node.left).__name__ == 'Variable':
            self.load_Variable(node.left, left)  # load variable from fp to left t
        else:
            # Either Bool or Num or BinaryOp or CompareOp
        ...
        if type(node.right).__name__ == 'Variable':
            self.load_Variable(node.right, right)  # load variable from fp to right t
    ...
    def visit_IfOp(self, node):
        ...
        self.block_stack.append(self.block_count)  # ADDED, WILL BE POPPED OR REMAIN IN STACK
        ...
        if node.else_statements:
            exit_block = self.get_block()  # POP ELSE BLOCK| WONT BE POPPED WILL REMAIN IN THE STACK TILL END
